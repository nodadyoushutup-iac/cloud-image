...
curl -fsSL https://download.docker.com/linux/packer/gpg | sudo gpg --dearmor -o /etc/apt/keyrings/docker.gpg || echo "[ERROR] Failed to add Docker's GPG key."

echo "[INFO] Setting up the Docker repository..."
echo \
    "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/packer \
